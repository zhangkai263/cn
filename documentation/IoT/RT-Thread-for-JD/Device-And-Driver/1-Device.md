...
![I/O 设备模型框架](media/5722a196503e198c48012a39f6354c8d.png)
...
![简单 I/O 设备使用序列图](media/4325ead0a144794832cc2b65745a6d92.png)
...
![看门狗设备使用序列图](media/6f2e2da19b42674c72ebaccb24f5a06b.png)
...
![设备继承关系图](media/2e5a2c697d54e75fd21b6ca2be0f403f.png)
...
![块设备](media/7362ca81b39b0fffa3b2865c397fab3d.png)

块设备

当系统服务于一个具有大量数据的写操作时，设备驱动程序必须首先将数据划分为多个包，每个包采用设备指定的数据尺寸。而在实际过程中，最后一部分数据尺寸有可能小于正常的设备块尺寸。如上图中每个块使用单独的写请求写入到设备中，头
3
...
![I/O 设备管理接口与 I/O 设备的操作方法的映射关系](media/28d0a45249940f7c3bd39f06b21bfb0e.png)
...
!!! note "注意事项" 当一个设备已经初始化成功后，调用这个接口将不再重复做初始化
0。
...
调用这个函数，会从 dev 设备中读取数据，并存放在 buffer

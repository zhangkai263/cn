...
注释：<br>
1、计算力是同一规格族内，反映单实例或多实例集群计算能力的指标，计算力= $\Sigma$ 规格尺寸* 数量（规格尺寸是在同一规格族内衡量规格计算能力的依据，CPU规格通常等于核数，GPU规格通常等于GPU卡数），如通用标准二代规格族g.n2下有2个2C8G实例和2个4C16G实例，则在该规格族下，4个实例的计算力为2×2 + 4×2=12。<br>

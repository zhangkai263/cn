...
用户在创建 VPC 时，可以按无类别域间路由块(CIDR，例如 10.0.0.0/16)的形式为 VPC 指定 IP 地址范围，也可以不预设CIDR(此时VPC的范围将随在其中创建的子网的CIDR范围而动态改变)。在创建支持IPv6 VPC时，京东智联云为您提供固定大小的（/56）IPv6 CIDR. 您无法选择IPv6 CIDR范围。私有网络有地域属性，用户无法跨地域创建 VPC 。不同VPC之间的IP地址块可以重叠。

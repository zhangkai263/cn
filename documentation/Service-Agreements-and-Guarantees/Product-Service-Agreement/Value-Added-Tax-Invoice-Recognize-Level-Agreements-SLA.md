...
本服务等级协议（Service Level Agreement，简称 "SLA"）规定了京东智联云向客户提供的增值税发票识别服务（以下简称"该服务"）的可用性等级指标及赔偿方案，向您承诺提供数据管理和业务质量方面的保障。同时，京东智联云有权根据变化适时对部分指标作出调整。若无特殊约定，本条款中的"年"均按自然年计算。
...
该服务是京东智联云开发的增值税发票识别服务。目前版本包括[识别增值税发票发票代码，发票号码，购方名称，购方税号等多个关键字段功能。所有功能均已在官网提供详细的功能介绍和使用说明文档。
...
用户免费使用增值税发票识别服务的部分，不予赔偿。

对于用户付费使用增值税发票识别服务的部分，根据增值税发票识别服务可用性，按如下方案进行赔偿：
...
赔偿调用次数不超过用户在服务月度内已购买增值税发票识别服务次数包的次数总额。

**本试用服务条款是京东云计算有限公司（本协议中称为“京东智联云”网址：www.jdcloud.com ）与您就分析型云数据库 JCHDB 相关事项所订立的有效合约。您通过盖章、网络页面点击确认或以其他方式选择接受本试用服务条款，包括但不限于未点击确认本试用服务条款而事实上使用了京东智联云分析型云数据库 JCHDB 试用服务，即表示您与京东智联云已达成协议并同意接受本试用服务条款的全部约定内容。如若双方盖章文本与网络页面点击确认或以其他方式选择接受之服务条款文本，存有不一致之处，以双方盖章文本为准。**
...
### 1. 服务内容
1.1本条款中“服务”指：京东智联云向您提供www.jdcloud.com 网站上所展示的分析型云数据库 JCHDB 试用服务以及相关的技术及网络支持服务。

1.2 京东智联云提供的服务必须符合本试用服务条款的约定。

### 2. 服务费用
2.1 公测期间，用户可免费试用分析型云数据库 JCHDB 。
...
3.2.3 若服务涉及分析型云数据库 JCHDB 的，京东智联云仅负责数据库系统以下的底层部分及京东智联云提供的软件的运营维护，即分析型云数据库 JCHDB 的相关技术架构及操作系统等。数据库之上的应用部分由您自行负责。
...
### 7. 期限与终止
7.1 服务期限自您分析型云数据库 JCHDB 创建成功之日起（而非自您获取分析型云数据库 JCHDB 的密码之日起）计算，并以您所缴纳的款项数额为依据确认服务期限。
...
7.2.3.1自首次欠费之时起，您仍可继续使用京东智联云分析型云数据库 JCHDB 服务24小时；如24小时届满，您仍未成功充值并足以支付所欠服务费用的，京东智联云将暂停提供服务、冻结您账户下分析型云数据库 JCHDB 的操作，但仍存储您的数据并持续计费；

...
- 分布式云物理服务器到期后不停服，数据保留7天，7天后系统将对分布式云物理服务器执行关机并且回收资源，数据无法找回；针对因到期释放的实例，会在释放时向您推送停服通知与释放通知。

- 您续费后可正常使用，新周期起始日为上一周期到期日后一天，资源需要您通过控制台手动进行“开机”操作。例如：上一到期日时间为2016年1月1日23:59:59，则续费后起始时间为为2016年月1日2日 00:00:00。
...
- 您的分布式云物理服务器欠费后，控制台界面将只能执行删除操作，不可执行开机、关机、重装系统等操作。请您务必注意查看通知并及时充值，以免造成不必要的损失；

- 分布式云物理服务器到期后不停服，数据保留7天，7天后系统将对分布式云物理服务器执行关机并且回收资源，数据无法找回；针对因到期释放的实例，会在释放时向您推送停服通知与释放通知。

- 您补缴欠费金额后可正常使用，资源需要您通过控制台手动进行“开机”操作。

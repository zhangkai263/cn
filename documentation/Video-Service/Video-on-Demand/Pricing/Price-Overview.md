...
累计的存储容量（100GB） * 对应存储的阶梯单价（0.00427）=0.427元
...
•	如您的月消费金额大于10万，可以使用CDN提供的月度95带宽峰值计费方式。   
### 计费实例
...
## D.视频剪辑
### 计费项
视频剪辑计费：视频裁剪、合并、剪辑等处理产生的费用，与转码计费一致，按照输出文件的编码方式、分辨率和时长计费。
### 计费价格
<table>
<tr>
    <td>编码方式<br/>
...
### 计费说明
•	付费方式：后付费。    
•	计费规则：根据剪辑输出的编码方式、分辨率和转码输出文件时长收费，以每天为计费周期。    
•	计费周期：按天计费，计费周期为0:00到24:00，计费生效时间戳为剪辑任务完成时间，如当天23:00提交剪辑任务，于次日0:05任务结束，则计入次日计费周期，每日中午 12 时至下午 18 时，对前一日产生费用进行结算，输出账单并进行扣费。  
•	输出时长：对于每个剪辑输出文件，按分钟计费，文件时长精确到小数点后两位，第二位根据第三位四舍五入，未满1秒的时长按0.02分钟计算。   
•   输出规格判定：按输出视频分辨率的长边和短边属于输出规格划定的范围进行判定，公式如下：以输出HD (1280 x 720)及以下规格为例，输出视频的分辨率长边不大于1280且短边不大于720则属于该输出规格。 如输出视频的长边大于1280或输出视频的短边大于720，该输出视频属于更高输出规格。    
•   计费公式： 视频剪辑输出文件时长（分钟）x 不同编码方式分辨率视频的转码单价（元/分钟）。    
### 计费实例
例如您在 6月 18 日使用点播视频编辑服务对视频 A（时长50分钟，分辨率1280 × 720）和视频 B（时长50分钟，分辨率1920 × 1280 ）进行拼接，输出视频为 C（默认编码格式H.264，时长100分钟，分辨率1280 × 720），故6月19日您需要支付的视频编辑费用如下：    
6月18日视频编辑费用 = 0.03（元/分钟）× 100（分钟）= 3（元）。

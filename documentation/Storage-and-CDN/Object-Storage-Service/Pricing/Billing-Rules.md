...
|外网下行流量|通过公网从京东智联云对象存储下载数据到本地端所产生的下行流量。|流量费会按天为单位，累计计费。通过IPv6协议访问产生的流量，均按照外网下行流量计费。<br>每天累计外网下行流量（GB）\*每GB单价。例如，北京地域某存储空间（Bucket），当天已累计产生外网下行流量100GB，所以外网下行流量费用为0.5\*100（GB）=50元|

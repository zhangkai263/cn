# 设置白名单
白名单控制哪些 IP 地址能够访问 JCHDB 实例。 默认情况下，实例所在的 VPC 段会在白名单列表中，即在该VPC中的所有IP都可以访问。建议只添加真正需要访问的IP地址，从而保证实例的数据安全性。修改后的白名单的会立即生效，不需要重启实例。白名单支持IPV6地址。
...
## 操作步骤
1. 登录 [分析型云数据库 JCHDB 控制台](https://jchdb-console.jdcloud.com)。
...
- 从云主机选择方式会显示所有和分析型云数据库JCHDB 实例处于同一VPC的云主机实例，您只要勾选需要访问云实例的云主机，它的内网 IP 地址会自动添加到白名单规则中。

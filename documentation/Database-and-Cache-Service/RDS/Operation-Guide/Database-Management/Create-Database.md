...
3. 点击目标实例，进入实例详情页，切换至 **库管理** Tab 页，点击 **创建库** 按钮，创建库弹窗框参数说明如下：
    * 库名称：库名我们保留了一些关键字名称，请参考 [限制说明](https://docs.jdcloud.com/cn/rds/mysql-restrictions)，并且库名的长度和字符有一定限制，具体以控制台为准。
    * 字符集：默认是 `utf8`，同时还提供了 `gbk`，`latin1`，`utf8mb4` , `euckr`,`armscii8` ,`ascii` ,`big5` 几个选项，可以按需选择。
...
2. 选择目标实例所在地域。
3. 点击目标实例，进入实例详情页，切换至 **库管理** Tab 页，点击 **创建库** 按钮，创建库弹窗框参数说明如下：
    * 库名称：库名我们保留了一些关键字名称，请参考 [限制说明](https://docs.jdcloud.com/cn/rds/sqlserver-restrictions)，并且库名的长度和字符有一定限制，具体以控制台为准。

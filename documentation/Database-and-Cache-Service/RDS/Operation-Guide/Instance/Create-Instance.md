...
- 地域：选择实例所在的地域， **不同地域资源的内网不互通，创建后不能更改** 。关于地域的详细说明，请参考  [核心概念](../../Introduction/Core-Concepts.md)。建议选择最靠近您的地域，可降低访问时延、提高下载速度。
...
- 存储类型：支持本地SSD盘、本地NVMe盘和SSD云盘, 详情请参考 [MySQL 产品规格](../../Introduction/Specifications/MySQL-Specifications.md)。
...
- 基本信息
   ...
   - 端口：目前 MySQL，Percona，MariaDB，SQL Server 支持指定运行端口
      - MySQL，Percona，MariaDB：默认值为 3306，端口范围支持 1150 ~ 5999。
      - SQL Server：默认值为 1433，端口范围支持 1150 ~ 5999，不支持5022。
...
## 6. 创建完成
稍等一会后，点击 **刷新** 按钮，手工刷新页面，显示实例创建完成，处于 **运行**状态。

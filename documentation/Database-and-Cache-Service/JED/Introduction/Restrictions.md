...
# Union after storing result in outfile
"select id from user into outfile s3 'out_file_name' union all select id from music"
"Incorrect usage/placement of 'INTO OUTFILE S3' (errno 1234) (sqlstate 42000)"

# Into outfile s3 in sub-query
"select id from (select id from user into outfile s3 'inner_outfile') as t2"
"Incorrect usage/placement of 'INTO OUTFILE S3' (errno 1234) (sqlstate 42000)"

# Multi shard query using into outfile s3
"select * from user order by id limit 100 into outfile s3 'out_file_name'"
"unsupported: non bypass query with into outfile s3"

"load data from s3 'x.txt' into table x"
"set bypass destination first"

...
|**name**|String| |镜像名称。|
|**platform**|String|CentOS|镜像的操作系统平台名称。<br>返回值：`Ubuntu、CentOS、Windows Server、Other Linux、Other Windows`。<br>|
|**osVersion**|String|8.2|镜像的操作系统版本。|
|**architecture**|String|x86_64|镜像架构。返回值：`x86_64、i386`。|
|**systemDiskSizeGB**|Integer|40|镜像系统盘大小。|
|**imageSource**|String|public|镜像来源，返回值：<br>`public`：官方镜像。<br>`thirdparty`：镜像市场镜像。<br>`private`：用户自己的私有镜像。<br>`shared`：其他用户分享的镜像。<br>`community`：社区镜像。<br>|
|**osType**|String|linux|镜像的操作系统类型。返回值：`windows、linux`。|
...
|**dataDisks**|[InstanceDiskAttachment[]](describeImages#instancediskattachment)| |镜像数据盘配置列表。|
...
|**localDisk**|[LocalDisk](describeImages#localdisk)| |本地磁盘配置，对应 `diskCategory=local`。|
|**cloudDisk**|[Disk](describeImages#disk)| |云硬盘配置，对应 `diskCategory=cloud`。|
...
|**diskType**|String| |云硬盘类型，返回值： ssd,premium-hdd,ssd.gp1,ssd.io1,hdd.std1|
...
|**tags**|[Tag[]](describeImages#tag)| | |
|**snapshotPolicies**|[SnapshotPolicy[]](describeImages#snapshotpolicy)| | |
...
|**contactInfo**|[ContactInfo](describeImages#contactinfo)| |联系人信息|
...
|名称|类型|示例值|描述|
|---|---|---|---|
|**chargeMode**|String| |支付模式，返回值：prepaid_by_duration，postpaid_by_usage或postpaid_by_duration，prepaid_by_duration表示预付费，postpaid_by_usage表示按用量后付费，postpaid_by_duration表示按配置后付费，默认为postpaid_by_duration|
|**chargeStatus**|String| |费用支付状态，返回值：normal、overdue、arrear，normal表示正常，overdue表示已到期，arrear表示欠费|
...
|**diskId**|String| |云硬盘ID|
|**instanceType**|String| |挂载实例的类型，返回值为 vm、nc|
|**instanceId**|String| |挂载实例的ID|
|**status**|String| |挂载状态，返回：为 "attaching", "attached", "detaching", "detached"|
|**attachTime**|String| |挂载时间|
### <div id="LocalDisk">LocalDisk</div>
|名称|类型|示例值|描述|
|---|---|---|---|
|**diskType**|String|ssd.gp1|磁盘类型，返回值：`ssd、premium-hdd、hdd.std1、ssd.gp1、ssd.io1`。|

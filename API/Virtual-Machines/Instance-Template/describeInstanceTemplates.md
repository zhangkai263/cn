...
|**filters**|[Filter[]](#filter)|否| |<b>filters 中支持使用以下关键字进行过滤</b><br>`name`: 实例模板名称，模糊匹配，支持多个<br>`instanceTemplateId`: 实例模板ID，精确匹配，支持多个<br>|
...
|名称|类型|示例值|描述|
|---|---|---|---|
|**instanceTemplates**|[InstanceTemplate[]](#instancetemplate)| |实例模板列表。|
...
|**encrypt**|Boolean| |是否是加密云盘。`false`：（默认）不加密。`true`：加密。|

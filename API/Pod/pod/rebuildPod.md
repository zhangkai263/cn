...
|名称|类型|是否必需|默认值|描述|
|---|---|---|---|---|
|**containers**|[RebuildContainerSpec[]](rebuildpod#rebuildcontainerspec)|True| |重置容器相关参数|
...
|**env**|[EnvSpec[]](rebuildpod#envspec)|False| |容器执行的环境变量；如果和镜像中的环境变量Key相同，会覆盖镜像中的值。长度范围：[0-100]|
...
|**livenessProbe**|[ProbeSpec](rebuildpod#probespec)|False| |容器存活探针配置|
|**readinessProbe**|[ProbeSpec](rebuildpod#probespec)|False| |容器服务就绪探针配置|
|**volumeMounts**|[VolumeMountSpec[]](rebuildpod#volumemountspec)|False| |云盘挂载信息|
...
|**exec**|[ExecSpec](rebuildpod#execspec)|False| |在容器内执行指定命令；如果命令退出时返回码为 0 则认为诊断成功。|
|**httpGet**|[HgSpec](rebuildpod#hgspec)|False| |对指定的端口和路径上的容器的 IP 地址执行 HTTP Get 请求。<br><br>如果响应的状态码大于等于200 且小于 400，则诊断被认为是成功的。 <br>|
|**tcpSocket**|[TcpSocketSpec](rebuildpod#tcpsocketspec)|False| |对指定端口上的容器的 IP 地址进行 TCP 检查；如果端口打开，则诊断被认为是成功的。|
...
|**httpHeader**|[HhSpec[]](rebuildpod#hhspec)|False| |自定义Http headers|
